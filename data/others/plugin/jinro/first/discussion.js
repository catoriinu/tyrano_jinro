 ...
 * @returns {array} array({number}最終確率, {boolean}判定結果)
 */
 ...
 */
 function zeronize (perspective, characterId, zeroRoleIds, distributeCharacterIds) {
    ...
    // TODO 視点の全役職を縦に確認し、「役職の未確定数が残っており、（あるキャラ以外は0か1になっており）あるキャラがその役職で確定する」状態だった場合に、
 ...
 * @returns {Array} 合法報告オブジェクトの配列。[{characterId, bool(t:●/f:○), 報告後の視点オブジェクト}, ...]
 */
 function bak_generateRegalAnnouncements (characterObject) {
    
    let regalAnnouncements = [];
    const fortuneTelledCharacterIds = characterObject.role.fortuneTellingHistory.map(resultObject => resultObject.characterId);
    const boolArray = [true, false];
    for (let i = 0; i < participantsIdList.length; i++) {

        // 自分の占い履歴にいるキャラクター、および自分自身は対象外
        if (fortuneTelledCharacterIds.includes(participantsIdList[i]) || participantsIdList[i] == characterId) continue;
        ...
        for (let j = 0; j < boolArray.length; j++) {
            ...
            try {
                regalAnnouncements.push(
                    [
                        participantsIdList[i],
                        boolArray[j],
                        organizePerspective(characterObject.perspective, participantsIdList[i], getRoleIdsForOrganizePerspective(boolArray[j]))
                    ]
                );
            } catch (zeronizeError) {
                console.log('zeronizeError : ' + participantsIdList[i] + ' is ' + boolArray[j]);
                continue;
            }
        }
    }
