...
*executeCharaNewAndCharaFace
  [iscript]
    tf.characterId = CHARACTER_ID_SORA;
    tf.jname = '九州そら';
    ...
    f.defaultPosition[tf.characterId] = {
      ...
      top: 96, // キャラが登場したときのtopの値
      widthCenter: 343, // 画像の幅の中央（立ち絵の見た目の中央の位置。画像の左端からのpxで指定）
      leftOnLeft: -71, // キャラが左側に登場したときのleftの値
      leftOnRight: 725, // キャラが右側に登場したときのleftの値
  ...
  [chara_new name="&tf.characterId" storage="chara/sora/normal.png" width="&f.defaultPosition[tf.characterId].width" haight="&f.defaultPosition[tf.characterId].haight" jname="&tf.jname" reflect="&f.defaultPosition[tf.characterId].reflect"]
  [chara_face name="&tf.characterId" face="通常" storage="chara/sora/normal.png"]
  [chara_show name="&tf.characterId" face="通常" time="0" wait="true" left="&f.defaultPosition[tf.characterId].leftOnDefautRight" top="&f.defaultPosition[tf.characterId].top" reflect="true"]
[return]
