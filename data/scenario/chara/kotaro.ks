...
*executeCharaNewAndCharaFace
  [eval exp="tf.characterId = CHARACTER_ID_KOTARO"]
  ...
  [iscript]
    f.defaultPosition[tf.characterId] = {
      ...
      side: 'left', // デフォルト待機位置
      left: -1096, // デフォルト座標（left）
      top: 171, // キャラが登場したときのtopの値
      width: 555, // 画像の幅（画面幅1280pxの中での幅）
      haight: 740, // 画像の高さ（画面高さ720pxの中での高さ）
      widthCenter: 243, // 画像の幅の中央(反転しない状態で)
    }
  ...
  [chara_new name="&tf.characterId" storage="chara/kotaro/normal.png" width="&f.defaultPosition[tf.characterId].width" haight="&f.defaultPosition[tf.characterId].haight" jname="白上虎太郎" reflect="true" ]
  [chara_face name="&tf.characterId" face="normal" storage="chara/kotaro/normal.png"]
