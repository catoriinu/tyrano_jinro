...
*executeCharaNewAndCharaFace
  [eval exp="tf.characterId = CHARACTER_ID_MESUO"]

  ; キャラ画像のデフォルト座標をゲーム変数に格納する
  [iscript]
    f.defaultPosition[tf.characterId] = {
      //side: 'right', // デフォルト待機位置
      //left: 1725, // デフォルト座標（left）
      side: 'left', // デフォルト待機位置
      left: -1071, // デフォルト座標（left）
  ...
  [chara_new name="&tf.characterId" storage="chara/mesuo/normal.png" width="&f.defaultPosition[tf.characterId].width" haight="&f.defaultPosition[tf.characterId].haight" jname="剣崎雌雄" reflect="true" ]
  [chara_face name="&tf.characterId" face="normal" storage="chara/mesuo/normal.png"]
