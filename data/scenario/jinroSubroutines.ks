...
; FIXME glinkFromCandidateObjectsの代わりにglinkFromButtonObjectsを使うように修正してください。移植が完了したら削除すること。
...
*loopstart_c
  ...
  [jump target="*loopend_c" cond="tf.cnt == (tf.buttonCount - 1)"]
  [eval exp="tf.cnt++"]
  [jump target="*loopstart_c"]
*loopend_c

[iscript]
  alert('glinkFromCandidateObjectsの代わりにglinkFromButtonObjectsを使うように修正してください');
  ; ボタンにカーソルが乗ったときの処理
  $(".buttonhover").hover(
    function(e) {
      ; ホバーしたボタンのclass属性の中から、ボタン生成時に付与しておいたId部分を抽出する
