...
- j_prepareJinroGame をプラグイン共通マクロ macro.ks に移し、参加者データの解決と Tyrano 変数の初期化をプラグイン内で完結するよう再構成しました。
- 昼夜開始時の初期化マクロ（j_daytimeInitialize / j_nightInitialize など）を同じ macro.ks に統合し、シナリオ側は共通マクロ呼び出しのみで状態遷移を制御できるようにしました。
- first/prepare.js のヘルパー構成と Participant / Character 生成順を見直し、デフォルトデータの複製や初期化処理を整理してメンテナンス性を向上させました。
...
### jinroプラグイン
#### バグ修正
- Characterインスタンスのpersonalityプロパティ生成時、性格名を明示しなかった場合に「テスト用の性格」が入ってしまうバグを修正

### リファクタリング
- ティラノスタジオの出力物などを追跡対象から外すよう`.gitignore`を整理
- 議論フェイズの信頼度判定処理に関するコメント表現を意図に合わせて修正

### ボイボ人狼
- ゲームのバージョンをver.1.0.1に更新
