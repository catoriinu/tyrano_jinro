...
- [ ] 【◎】ステータス画面作成
  - [x] 【★】退場済みキャラクターは敗北時の立ち絵を表示する
  - [x] 【★】勝敗がついてから表示するときには、勝利時敗北時引き分け時の立ち絵を表示する
  - [x] 【★】勝敗がついてから表示するときには、役職を全公開する
...
- [x] 【◎】メニュー画面作成
  ...
  - [ ] ボイボ人狼正式版ではボタン表示させない
- [ ] 【★】ゲーム終了時の役職紹介画面作成
  - displayCharactersHorizontallyサブルーチンを利用したい。というよりステータス画面と同じような方式にしたい→した
  - 「ボタン以外をクリックでタイトルに戻る」ようにする→しないことにした。通常は勝利陣営だけ表示し、詳細はステータス画面を開かせる。
  - [x]【★】勝利時敗北時引き分け時の立ち絵を定義しておく（勝利時は陣営ごと）
  - [x]【★】勝利陣営は勝利時用の立ち絵を表示する
  - [ ]【★】表示する立ち絵にふさわしい差分を準備する（わかりやすく喜んでいる、悲しんでいる）
- [ ] 【☆】ボイボ人狼正式版用タイトル画面作成
- [x] 【◎】処刑時、襲撃時の演出作成→今以上はとりあえず要らないと思う
- [ ] 【○】キャラクターの性格を作る
- [ ] 【○】システムメッセージにもボイスを入れる
...
- [x] 【◎】ボイスのプリロードはゲーム起動時に行う（方がいいか含め検討する）→起動時ではなく開始時のままでいい。preload済みならロード時間かからないので繰り返しプレイには影響ない。
  - [x] 【○】NOW LOADING画面を作る→[loadind_log]タグをfirst.ksで実行して解決。
- [ ] バグ修正
