...
- [ ] COフェイズ
  ...
  - [x] 【○】PCの騙り占い結果選択時、アクションボタンと同じように、第1階層はキャラクター、第2階層は騙り結果、という構成にしたい(fakeFortuneTellingForPC)
  - [x] 【☆】PCの騙り占い結果選択時、「一つ前に戻る」ボタンを追加して入力を取り消せるようにする
  - [ ] 【△】CO時のセリフパターンの追加
...
- [x] 【◎】ボイスのプリロードはゲーム起動時に行う（方がいいか含め検討する）→起動時ではなく開始時のままでいい。preload済みならロード時間かからないので繰り返しプレイには影響ない。
  - [x] 【○】NOW LOADING画面を作る→[loading_log]タグをfirst.ksで実行して解決。
- [ ] バグ修正
  ...
  - [ ] プレビューでのプレイ開始時に、ボイス音量が大きく感じる（sfのボイス音量が適用されていない？）
- [ ] 【◎】ゲームシナリオ作る
- [ ] 【◎】遊びの部分を作る
  - [x] ステージ選択
  - [ ] 【★】会話劇
...
- [ ] 【☆】コンフィグ画面作成
  - [x] 【☆】作成する
  - [x] 【☆】戻るボタンを作ってコンフィグ画面に置く
  - [ ] 【☆】コンフィグ画面をメニューから開けるようにする
